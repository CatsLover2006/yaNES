...
#define decode_instruction {\
    ...
    if (instruction == BRK) {\
        if (resetCPU) t16 = 2;\
        else if (recievedNMI) t16 = 1;\
        else if (recievedIRQ) t16 = 4;\
        else t16 = 0;\
    }\
}
...
void CPU::doCycle() {
    ...
    // std::cout << pc << ";";
    if (subCycle == 0) {
        // TODO: verify instruction decoding logic
        decode_instruction;
        pc++;
    } else doInstruction();
    // std::cout << pc << ";" << (u16) accumulator << ";" << (u16) x << ";" << (u16) y << ";" << (u16) status << ";" << (u16) sp << ";" << (u16) instruction << ";" << (u16) subCycle << ";" << (u16) resetCPU << ";" << (u16) recievedNMI << ";" << (u16) recievedIRQ << std::endl;
    resetCPU &= 1;
    recievedNMI &= 1;
    recievedIRQ &= 1;
...
#define macro_indexed_indirect_read(inst, index) {\
    switch(subCycle) {\
        case 1: {\
            t = memory.read(pc);\
            pc++;\
...
#define macro_indirect_indexed_read(inst, index) {\
    switch(subCycle) {\
        case 1: {\
            t = memory.read(pc);\
            pc++;\
    ...
    switch(subCycle) {\
        ...
        case 3: {\
            ...
            if (t) {\
                ...
                subCycle = 0;\
                pc++;\
            } else subCycle = FINISHED_INSTRUCTION;\
            return;\
        }\
    }\
}

#define macro_zero_manip(inst) {\
    switch(subCycle) {\
        case 1: {\
            t16 = memory.read(pc);\
            pc++;\
...
#define macro_zero_indexed_manip(inst, index) {\
    switch(subCycle) {\
        case 1: {\
            t = memory.read(pc);\
            pc++;\
...
#define macro_abs_indexed_manip(inst, index) {\
    switch(subCycle) {\
        case 1: {\
            t16 = memory.read(pc);\
            pc++;\
        ...
        case 3: {\
            t16 += index;\
            if (t16 >> 8 != t) memory.read(t16 - 0x0100);\
            else memory.read(t16);\
            return;\
        }\
        case 4: {\
            t = memory.read(t16);\
...
#define macro_indexed_indirect_manip(inst, index) {\
    switch(subCycle) {\
        case 1: {\
            t = memory.read(pc);\
            pc++;\
...
#define macro_indirect_indexed_manip(inst, index) {\
    switch(subCycle) {\
        case 1: {\
            t = memory.read(pc);\
            pc++;\
...
void CPU::doInstruction() {
    ...
    switch (instruction) {
        case BRK: // TODO: verify interrupts
            if (!t16) {
                setStatusB;
                if (recievedNMI || recievedIRQ) {
                    if (recievedNMI) t16 = 1;
                    else t16 = 4;
                    subCycle = 0;
                    return;
                }
        ...
        case ROR: macro_impl({
            ...
            accumulator >>= 1;
            if (instruction == ROR && statusC) accumulator |= 0x80;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        });
        ...
        case LAX_0: macro_zero_read(x = t; accumulator = x; updateZN(x));
        case BIT_0: macro_zero_read({
            status = status & ~(statusN & statusV);
            status |= t & 0x80;
            status |= (t >> 5) & 0x01;
        ...
        case ASL_0:
        case ROL_0: macro_zero_manip({
            if (accumulator & 0x80) t = true;
            else t = false;
            accumulator <<= 1;
            if (instruction == ROL_0 && statusC) accumulator |= 1;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        });
        case LSR_0:
        case ROR_0: macro_zero_manip({
            if (accumulator & 1) t = true;
            else t = false;
            accumulator >>= 1;
            if (instruction == ROR_0 && statusC) accumulator |= 0x80;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        });
        case INC_0: macro_zero_manip(t++; updateZN(t));
        case DEC_0: macro_zero_manip(t--; updateZN(t));
        case ADC_0X:
        ...
        case ASL_0X:
        case ROL_0X: macro_zero_indexed_manip({
            if (accumulator & 0x80) t = true;
            else t = false;
            accumulator <<= 1;
            if (instruction == ROL_0X && statusC) accumulator |= 1;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        }, x);
        case LSR_0X:
        case ROR_0X: macro_zero_indexed_manip({
            if (accumulator & 1) t = true;
            else t = false;
            accumulator >>= 1;
            if (instruction == ROR_0X && statusC) accumulator |= 0x80;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        }, x);
        case INC_0X: macro_zero_indexed_manip(t++; updateZN(t), x);
        case DEC_0X: macro_zero_indexed_manip(t--; updateZN(t), x);
        case ADC_A:
        ...
        case LAX_A: macro_abs_read(x = t; accumulator = x; updateZN(x));
        case BIT_A: macro_abs_read({
            status = status & ~(statusN & statusV);
            status |= t & 0x80;
            status |= (t >> 5) & 0x01;
            updateZ(t & accumulator);
        ...
        case ASL_A:
        case ROL_A: macro_abs_manip({
            if (accumulator & 0x80) t = true;
            else t = false;
            accumulator <<= 1;
            if (instruction == ROL_A && statusC) accumulator |= 1;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        });
        case LSR_A:
        case ROR_A: macro_abs_manip({
            if (accumulator & 1) t = true;
            else t = false;
            accumulator >>= 1;
            if (instruction == ROR_A && statusC) accumulator |= 0x80;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        });
        case INC_A: macro_abs_manip(t++; updateZN(t));
        case DEC_A: macro_abs_manip(t--; updateZN(t));
        case ADC_AX:
        ...
        case ASL_AX:
        case ROL_AX: macro_abs_indexed_manip({
            if (accumulator & 0x80) t = true;
            else t = false;
            accumulator <<= 1;
            if (instruction == ROL_AX && statusC) accumulator |= 1;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        }, x);
        case LSR_AX:
        case ROR_AX: macro_abs_indexed_manip({
            if (accumulator & 1) t = true;
            else t = false;
            accumulator >>= 1;
            if (instruction == ROR_AX && statusC) accumulator |= 0x80;
            if (t) setStatusC;
            else clearStatusC;
            updateZN(accumulator);
        }, x);
        case INC_AX: macro_abs_indexed_manip(t++; updateZN(t), x);
        case DEC_AX: macro_abs_indexed_manip(t--; updateZN(t), x);
        ...
        case AND_I: macro_indexed_indirect_read(accumulator &= t; updateZN(accumulator), x);
        case ORA_I: macro_indexed_indirect_read(accumulator |= t; updateZN(accumulator), x);
        case CMP_I: macro_indexed_indirect_read({
            updateZN(accumulator - t);
            if (statusN) clearStatusC;
            else setStatusC;
        }, x);
        ...
        case AND_IY: macro_indirect_indexed_read(accumulator &= t; updateZN(accumulator), y);
        case ORA_IY: macro_indirect_indexed_read(accumulator |= t; updateZN(accumulator), y);
        case CMP_IY: macro_indirect_indexed_read({
            updateZN(accumulator - t);
            if (statusN) clearStatusC;
            else setStatusC;
        }, y);
        ...
        case BCC: macro_branch(!statusC);
        case BCS: macro_branch(statusC);
        case BNE: macro_branch(!statusZ);
        case BEQ: macro_branch(statusZ);
        case BPL: macro_branch(!statusN);
        case BMI: macro_branch(statusN);
        case BVC: macro_branch(!statusV);
        case BVS: macro_branch(statusV);
        case NOP:
        case 0x1A:
        case 0x3A:
        case 0x5A:
